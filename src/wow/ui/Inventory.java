...
public class Inventory extends WoWscreen {
	...
	System.err.println("Inventory.activate() active="+active);
	if (active)
	    WoWgame.self().fireFgSound(WoWmobile.resource("/sounds/universal/TradeskillLearnRecipeLoop.mp3"));
    }

    public void sizeEvent(int width, int height) {
	System.err.println("Inventory.sizeEvent() "+width+" x "+height);
	m_width = width;
	m_buttonY = height - 18;
	int n = (width-2) / 34;
	...
	m_buttonCount = n;
	WoWgame.self().setDebug("Inventory buttons: "+n);
    }
	...
	System.err.println("Inventory.keyEvent() key="+key+" act="+action+" press="+pressed);
