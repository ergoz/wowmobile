...
{
    private static final byte[] decSeed = {
    ...
    private static final byte[] encSeed = {
    ...
    {
	//System.err.println("WoWconn.readByte()");
	int c = iStream.read();
	if (c < 0)
	    throw new IOException("Server closed connection");
	if (dCipher == null)
	    return c;
	// only the header (read one byte at a time) may be encrypted
	byte pad[] = new byte[1];
    ...
    {
	//System.err.println("WoWconn.readByte() "+len);
	if (len <= 0)
	    throw new IOException("Invalid read length: "+len);
	byte b[] = new byte[len];
	int pos = 0;
	while (pos < len) {
	...
	// packet data is never encrypted
	return b;
    }
	...
	try {
	    ...
	    int len = readByte();
	    if ((len & 0x80) != 0)
		len = ((len & 0x7f) << 16) | (readByte() << 8) | readByte();
	    else
		len = (len << 8) | readByte();
...
*/
	...
	if (pkt == null || pkt.code() != WoWpacket.SMSG_AUTH_CHALLENGE)
	    return pkt;
	...
	try {
	    ...
	    auth = append(auth,unk);
	    if (!writePacket(WoWpacket.CMSG_AUTH_SESSION,auth))
		return null;
