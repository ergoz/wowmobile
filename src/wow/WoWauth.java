...
//
// Authentication socket handling
...
public class WoWauth
{
	...
	if (oStream == null || userName.length == 0 || userName.length > 32)
	    return false;
	int tzo = 120;
	...
	for (len = 0; len < userName.length; len++)
	    b[34+len] = userName[len];
	return writeByte(b);
    }
	...
	for (int i = 0; i < cnt; i++) {
	    ...
	    if (chars > 0)
		realmList.addElement(new WoWrealm(name,addr));
	}
	readWord();
	realmsEof = true;
	return true;
